...
DragForceList = []
DragForceReductionList = []
MeanDragForceReduction = 1.0

# Dimensions of Simulation Domain
zOffset = 0.1		# distance between front and back plane 
xLeft=-3
xRight= 5
yTop=5
yBottom=0
...
# Parameters controlling the updation of shape in each iterations
minWeight = 1e-5
coordTol = 0.001

# folder multiples to be retained and other folders will be deleted 
folderMultiples = 50 


stoppingCriteria = 1e-5		# = mean(Iteration_n+1 - Iteration_n) / InitialDragForce
maxIter = 800				# maximum number of iterations permitted
Iter =0						# Iteration counter

while (MeanDragForceReduction > stoppingCriteria) and Iter<maxIter:
	if Iter%folderMultiples ==1 and Iter!=1:
		folderNo = Iter-folderMultiples
		while folderNo%folderMultiples!=0:
			if Path(Itpath+str(folderNo)).exists() == True and folderNo!=1:
				shutil.rmtree(Itpath+str(folderNo))
			folderNo +=1	
	if Path(Itpath+str(Iter+1)).exists() == False:
		os.mkdir('Iteration_'+str(Iter+1))
		print('Starting Iteration '+str(Iter+1)+'.....')
		os.system('cp -r '+Refpath+'/* '+Itpath+str(Iter+1))
		os.chdir(Itpath+str(Iter+1)+'/NS')
		...
		if blockMeshDict:
				if Iter != 0:
						...
						for vertex in VertexList:
								f = open(Itpath+str(Iter+1)+"/NS/system/blockMeshDict", "r")
								for line in f:
								...
								if flag == True:
										s = open(Itpath+str(Iter+1)+"/NS/system/blockMeshDict").read()
										StripVertex = ' '.join(map(str, Vertices[index]))
										s = s.replace(replaceline, '(' + StripVertex + ') ' + findText + '\n')
										f = open(Itpath+str(Iter+1)+"/NS/system/blockMeshDict", 'w')
										f.write(s)
										f.close()
										index = index + 1
										flag = False
				...
				print('Iter-'+str(Iter+1)+' (NS): Meshing done and checked!')
				print('Iter-'+str(Iter+1)+' (NS): Running solver...')
				os.system('simpleFoam > outputNS.txt')
				print('Iter-'+str(Iter+1)+' (NS): End of solution!')
		else:
				print('Block Mesh Dictionary is not available')
				sys.exit(1)
		endTimeNS = '0'
		iterationNSFolder = os.listdir(Itpath+str(Iter+1)+'/NS')
		...
		print('endtime of NS is ',endTimeNS)
		os.system('cp '+Itpath+str(Iter+1)+'/NS/'+endTimeNS+'/p '+Itpath+str(Iter+1)+'/OS/0')
		os.system('cp '+Itpath+str(Iter+1)+'/NS/'+endTimeNS+'/U '+Itpath+str(Iter+1)+'/OS/0')
		
		dragForceFile   = open(Itpath+str(Iter+1)+'/NS/postProcessing/forces/0/forces.dat','r')
		...
		print('Iter-'+str(Iter+1)+': Writing drag force to file..!')
		
		forceResultfile.write(str(Iter+1)+"\t"+str(DragForceX)+"\n")
		
		DragForceList.append(DragForceX)
	
		if Iter==0:
			InitialDragForce = DragForceX
			
		elif Iter==1:
			DragForceReductionList.append(DragForceList[-2]-DragForceList[-1])
			DragForceReductionList.append(DragForceList[-2]-DragForceList[-1])
			MeanDragForceReduction = np.average(DragForceReductionList[-10:])/InitialDragForce
			if MeanDragForceReduction <= stoppingCriteria:
				
				MeanDragForceReduction = 1.0
				print('Average Reduction in Drag Force is most likely negative and being corrected')
		else:
			DragForceReductionList.append(DragForceList[-2]-DragForceList[-1])
			MeanDragForceReduction = np.average(DragForceReductionList[-10:])/InitialDragForce
		
			if MeanDragForceReduction <= stoppingCriteria and Iter<20:
				
				MeanDragForceReduction = 1.0
				print('Most likely drag force is being increased but surpassing stoppingcriteria until 20 simulations!!')
			elif MeanDragForceReduction <= stoppingCriteria and Iter>20 and DragForceReductionList[-1]>0:
				MeanDragForceReduction = 1.0
				print('Drag force decreasing, surpassing stoppingcriteria for few more simulations!')
		print('Average Reduction in Drag Force is ', MeanDragForceReduction)
		f = fileinput.FileInput(Itpath+str(Iter+1)+'/OS/0/UAdj', inplace=True)
		for line in f:
		...
		os.chdir(Itpath+str(Iter+1)+'/OS')
		os.system('cp '+Itpath+str(Iter+1)+'/NS/system/blockMeshDict '+Itpath+str(Iter+1)+'/OS/system/')
		os.system('blockMesh >outputblockMesh.txt')
		os.system('checkMesh >outputcheckMesh.txt')
		
		
		print('Iter-'+str(Iter+1)+' (OS): Meshing done and checked!')
		print('Iter-'+str(Iter+1)+' (OS): Running solver...')
		os.system('pisomosiFoam >outputOS.txt')
		print('Iter-'+str(Iter+1)+' (OS): End of solution!')

		# Calculation of new boundary Coordinates
		print('Iter-'+str(Iter+1)+': Calculating new boundary points...')
		...
		word = []
		with open (Itpath+str(Iter+1)+'/OS/BoundaryResults.txt') as file:
				for line in file:
		...
		boundaryFile = open(Itpath+str(Iter+1)+'/NS/constant/polyMesh/boundary','r')
		boundaryFound = False
		...
		while weight > minWeight:
				...
				withinTolFlag = np.allclose(updated_global_pos,global_pos,rtol=coordTol, atol=1e-08, equal_nan=False)
		...
		for coordindex in range (len(updated_global_pos)):
				if updated_global_pos[coordindex] > (global_pos[coordindex]*(1+coordTol)):
						coordCount = coordCount +1
						updated_global_pos[coordindex] = (global_pos[coordindex]*(1+coordTol))
				elif updated_global_pos[coordindex] < (global_pos[coordindex]*(1-coordTol)):
						coordCount = coordCount +1
						updated_global_pos[coordindex] = (global_pos[coordindex]*(1-coordTol))
		print(str(coordCount)+' Coordinates constrained back to the tolerance limit '+str(coordTol))
		...
		os.chdir(Modpath)
		print('End of Iteration '+str(Iter+1)+'.')
		print('\n')
		Iter+=1
